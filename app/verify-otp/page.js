...
export default function VerifyOTP() {
  ...
  const recordFailedAttempt = async (voterId) => {
    ...
    if (newFailedCount >= 5) {
      ...
      await supabase.from('ip_locks').upsert({
        ip_address: clientIP,
        is_locked: true,
  ...
  const resetFailedAttempts = async (voterId) => {
    ...
    await supabase.from('ip_locks').delete().eq('ip_address', clientIP);
  };
  ...
  const generateNewOtp = async (voterId, email, schoolId, name) => {
    ...
    // Check if a record exists for this voter
    ...
    if (existingRecord) {
      // UPDATE existing record (avoids unique constraint)
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      if (existingOtp && new Date(existingOtp.expires_at) > now && existingOtp.otp_code) {
        ...
        // No valid OTP or expired – generate new one (UPSERT)
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      setTimeout(() => { verificationLock.current = false; }, 500);
    }
  };
  ...
  return (
    <>
      ...
      <button onClick={toggleTheme} className="fixed top-4 right-4 z-50 p-3 rounded-full bg-white/10 backdrop-blur-lg border border-white/20 hover:scale-110 transition-all duration-300" aria-label="Toggle theme">
        ...
        <div className="relative w-full max-w-md">
          <div className={`${currentTheme.cardBg} border ${currentTheme.cardBorder} rounded-3xl shadow-2xl p-6 sm:p-8 transform transition-all duration-300`}>
            ...
            <div className="mb-6 sm:mb-8">
              ...
              <div className="flex justify-center gap-2 sm:gap-3 flex-wrap">
                {otpDigits.map((digit, index) => (
                  <input key={index} ref={(el) => (inputRefs.current[index] = el)} type="text" inputMode="numeric" maxLength={1} value={digit} onChange={(e) => handleDigitChange(index, e.target.value)} onKeyDown={(e) => handleKeyDown(index, e)} onPaste={index === 0 ? handlePaste : undefined} disabled={isVerifying || isLocked} className={`w-10 h-12 sm:w-14 sm:h-16 text-center text-xl sm:text-2xl font-bold ${currentTheme.inputBg} border-2 rounded-xl ${currentTheme.textPrimary} focus:outline-none focus:ring-2 transition-all duration-200 ${digit ? `border-${theme === 'dark' ? '[#f4a261]' : 'teal-500'} bg-white/20 ring-1 ring-${theme === 'dark' ? '[#f4a261]' : 'teal-500'}/50` : currentTheme.inputBorder} ${currentTheme.inputFocus} ${isLocked ? 'opacity-50 cursor-not-allowed' : ''}`} autoFocus={index === 0 && !isLocked} />
                ))}
              </div>
            ...
            <button onClick={handleVerify} disabled={isVerifying || otpDigits.join('').length !== 6 || isLocked} className={`w-full py-3 sm:py-4 px-6 bg-gradient-to-r ${currentTheme.buttonPrimary} disabled:from-gray-600 disabled:to-gray-600 text-white font-semibold rounded-xl shadow-lg transform transition-all duration-300 hover:scale-105 disabled:transform-none disabled:cursor-not-allowed mb-4`}>
              {isVerifying ? <div className="flex items-center justify-center space-x-2"><FaSpinner className="w-4 h-4 sm:w-5 sm:h-5 animate-spin" /><span>Verifying...</span></div> : <div className="flex items-center justify-center space-x-2"><FaCheckCircle className="w-4 h-4 sm:w-5 sm:h-5" /><span>Verify & Vote</span></div>}
            </button>
            <div className="text-center mb-6">
              <button onClick={handleResendOtp} disabled={resendCooldown > 0 || isLoading || isLocked} className={`${theme === 'dark' ? 'text-[#f4a261] hover:text-[#e76f51]' : 'text-teal-600 hover:text-teal-700'} text-xs sm:text-sm disabled:opacity-50 transition-colors flex items-center justify-center gap-2 mx-auto`}>
