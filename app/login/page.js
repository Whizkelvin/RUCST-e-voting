...
function LoginContent() {
  ...
  // Role configuration - WITH ICON PROPERTY
  ...
  const getRoleIcon = useCallback(() => {
    ...
    if (roleConfig) {
      ...
      return <IconComponent className={getIconColor()} />;
    }
    return <FaUserShield className={getIconColor()} />;
  }, [adminRole, getRoleConfig, theme]);
  ...
  const checkVotingPeriod = useCallback(async () => {
    try {
      ...
      if (!settings.is_active) {
        ...
        message = 'Voting period has ended. Go to home page and view results.';
  ...
  const themeStyles = useMemo(() => ({
    dark: {
      ...
      statusCard: {
        checking: 'bg-gray-500/20 border-gray-400/50',
        voter: 'bg-gray-500/20 border-gray-400/50',
        admin: {
          dean: 'bg-gray-500/20 border-gray-400/50',
          electoral: 'bg-gray-500/20 border-gray-400/50',
          it_admin: 'bg-gray-500/20 border-gray-400/50',
          hod: 'bg-gray-500/20 border-gray-400/50',
    ...
    light: {
      ...
      statusCard: {
        ...
        voter: 'bg-gray-100 border-gray-300',
        admin: {
          dean: 'bg-gray-100 border-gray-300',
          electoral: 'bg-gray-100 border-gray-300',
          it_admin: 'bg-gray-100 border-gray-300',
          hod: 'bg-gray-100 border-gray-300',
    ...
    <>
      ...
      <div className={`min-h-screen bg-gradient-to-br ${currentTheme.background} flex items-center justify-center p-4 relative overflow-hidden transition-all duration-300`}>
        ...
        <div className="absolute inset-0">
          <div className={`absolute top-[-100px] right-[-100px] w-[300px] h-[300px] ${theme === 'dark' ? 'bg-gray-600' : 'bg-gray-400'} opacity-20 blur-3xl rounded-full animate-pulse`}></div>
          <div className={`absolute bottom-[-100px] left-[-100px] w-[300px] h-[300px] ${theme === 'dark' ? 'bg-gray-500' : 'bg-gray-300'} opacity-20 blur-3xl rounded-full animate-pulse delay-1000`}></div>
        </div>
        ...
        <div className="relative w-full max-w-md">
          ...
          >
            ...
            {/* Voting Period Status Banner - Grayscale */}
            {votingStatus && (
              <div className={`mb-4 p-3 rounded-xl ${
                ...
                <div className="flex items-center gap-2 justify-center">
                  {votingStatus.hasEnded ? (
                    ...
                    <FaCheckCircle className={getIconColor()} />
                  ) : (
                    <FaExclamationTriangle className={getIconColor()} />
                  )}
            ...
            >
              ...
              <div className="relative group">
                <FaEnvelope className={`absolute left-3 top-1/2 -translate-y-1/2 ${theme === 'dark' ? 'text-white/50 group-focus-within:text-gray-400' : 'text-gray-400 group-focus-within:text-gray-500'} transition`} />
                <input
                  {...register('email', {
              ...
              {/* School ID Field */}
              <div className="relative group">
                <FaIdCard className={`absolute left-3 top-1/2 -translate-y-1/2 ${theme === 'dark' ? 'text-white/50 group-focus-within:text-gray-400' : 'text-gray-400 group-focus-within:text-gray-500'} transition`} />
                <input
                  {...register('schoolId', {
