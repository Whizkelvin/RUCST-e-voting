...
const handleVoterLogin = useCallback(async (email, schoolId) => {
  ...
  try {
    ...
    // If valid OTP exists, redirect to verification
    ...
    // ========== STEP 5: NO VALID OTP - GENERATE NEW ONE ==========
    const otpCode = Math.floor(100000 + Math.random() * 900000).toString();
    const otpExpiry = new Date(Date.now() + 15 * 60 * 1000);
    const hashedOtp = await hashCode(otpCode);
    
    // Try to UPDATE existing record first (avoids unique constraint error)
    ...
    if (existingRecord) {
      ...
      otpError = insertError;
    }
    
    if (otpError) {
      console.error('OTP operation error:', otpError);
      toast.error('Failed to generate OTP. Please try again.');
      setIsLoading(false);
      return;
    }
